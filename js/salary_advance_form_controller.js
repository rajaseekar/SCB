...
function validate_account_number(accNumber, app)
{
    // filter out unnecessary characters. (e.g. - (dash))
    accNumber = accNumber.replace(/-/g, "");
    
    var len = accNumber.length;
    var formatlen = get_format(app).length;
    ...
    return eval_formula(get_formula(app), get_format(app), accNumber);
}

function get_format(app)
{
    var format = "";
...
/* */
function get_formula(app)
{
    var formula = "";
    
    if (app == "DACC")
    {
        formula = "B*2 + B*9 + B*8 + S*7 + S*6 + S*5 + S*4 + S*3 + S*2 + C*0";
    }    
...
/* */
function eval_formula(formula, format, accNumber)
{
    var formatLen = format.length;
    var formulaLen = formula.length;
    var CPos = -1;
    var divider = 11;
    ...
    for (var i=0; i<formatLen; i++)
    {
        ...
        {
            CPos = i; // Acquire the C value                
        }
        
        formula = formula.replace(format.charAt(i)+"", accNumber.charAt(i)+"");
    }  

    // Get remainder from the SUM()/divider
    var mod = eval(formula)%divider;
    ...
    // If remainder matches the C value found from input.
    if (C + "" == accNumber.charAt(CPos) + "")
    {
        return true;
    }
...
//	commonXML=commonXML+"\t\t\t<form2_submit_cpf><![CDATA["+$('input[name=form2_submit_cpf]:checked').val()+"]]></form2_submit_cpf>\n";
	...
	if ($("input[name='form2_os_fund_transfer']").is(":checked")) {	
		...
		commonXML=commonXML+"\t\t\t<form2_os_fund_transfer></form2_os_fund_transfer>\n";	
	...
	if ($("input[name='form2_os_link_debit_card']").is(":checked")) {	
		...
		commonXML=commonXML+"\t\t\t<form2_os_link_debit_card></form2_os_link_debit_card>\n";	
	...
	if ($("input[name='form2_os_go_green']").is(":checked")) {	
		...
		commonXML=commonXML+"\t\t\t<form2_os_go_green></form2_os_go_green>\n";	
	}
	
	if ($("input[name='form2_submissionType']:checked").val()=="P") {	
		commonXML=commonXML+"\t\t\t<form2_submissionType><![CDATA[I will print the completed form myself, then sign and mail it to Standard Chartered.]]></form2_submissionType>\n";	
	} else {
		commonXML=commonXML+"\t\t\t<form2_submissionType><![CDATA[Please send me my completed form. I will sign and mail it back to Standard Chartered.]]></form2_submissionType>\n";	
	}	
...
*/	
	...
	$.validator.addMethod("alphanumericwith2spec", function(value, element) {
		...
		return result;
	}, "Please enter only alphanumerics, # or -.");
	...
	var result = this.optional(element) || /^\#[a-zA-Z0-9]{1,4}-[a-zA-Z0-9]{1,7}$/.test(value);
	return result;
	}, "Please enter unit number in #XXX-XXX format.");

	$.validator.addMethod("validAccount", function( value, element) {
	var result = this.optional(element) || validate_account_number(value,"DACC");
	return result;
	}, "Please enter a valid savings/current account number.");
	...
	$("#form_notes_information").validate({
  		...
  		rules: {
			...
			form2_mobile: { required: true, numberonly: true, minlength: 8, maxlength: 8, validMobile: true },
			form2_residential: { required: function() { return ( $("#form2_residential").val() == "" && $("#form2_office").val() == "") }, numberonly: true, minlength: 8, maxlength: 8, validPhone: true },
			form2_office: { required: function() { return ( $("#form2_residential").val() == "" && $("#form2_office").val() == "") }, numberonly: true, minlength: 8, maxlength: 8, validPhone: true },
...
//Terence		
			...
			form2_nric_number: { required: "#form2_nric_number:visible", minlength: 9, maxlength: 9, validNRIC: true },
	...
	// --- address info
			...
			form2_length_of_stay_at_address_months: { required: true, minlength:1, maxlength:2, numberonly: true, min: 0, max: 12 },
...
*/	
	...
	$("#form2_os_account_type").change( function() {
		if(this.value == "Credit Card") {
			if( $("form2_os_bank").val() == 'Standard Chartered') {
				$("form2_os_bank").val("");
			}
			$("#form2_os_bank option[value='Standard Chartered']").remove();
		} else {
			if( $("#form2_os_bank option[value='Standard Chartered']").length == 0) {
				$('<option value="Standard Chartered">Standard Chartered</option>').appendTo("#form2_os_bank");
			}
		}
	});
	...
	$("#form_new_customer_credit_cards_4").validate({
  		rules: {
			...
			form2_os_nric: { alphanumericwithspec: true, minlength: 9, maxlength:9, validNRIC: true},
			...
			form2_os_transfer_amount: {required: "#form2_os_transfer_amount:visible", numberonly: true, maxlength:15, min: 500},
			form2_os_saving_current_account: {required: "#form2_os_saving_current_account:visible", accountNo: true, maxlength:16, validAccount: true}
  		},		
		messages: {
...
//		$("#pre_form2_submit_cpf").html($("input[name='form2_submit_cpf']:checked").val());
		...
		if( $("input[name='form2_mailing_address']:checked").val() == "Home" ) {
			...
			$("#pre_form2_mailing_unit_number").html($("#form2_unit_number").val());
			$("#pre_form2_mailing_street_name").html($("#form2_street_name").val());
			$("#pre_form2_mailing_building_name").html($("#form2_building_name").val());
			$("#pre_form2_mailing_postal_code").html($("#form2_postal_code").val());
		} else if( $("input[name='form2_mailing_address']:checked").val() == "Business" ) {
			$("#pre_form2_mailing_block_number").html($("#form2_employer_block_number").val());
			$("#pre_form2_mailing_unit_number").html($("#form2_employer_unit_number").val());
			$("#pre_form2_mailing_street_name").html($("#form2_employer_street_name").val());
			$("#pre_form2_mailing_building_name").html($("#form2_employer_building_name").val());
			$("#pre_form2_mailing_postal_code").html($("#form2_employer_postal_code").val());
		} else if( $("input[name='form2_mailing_address']:checked").val() == "Other" ) {
